...
const newContainer = document.querySelector('.New_product');

function createNewCard(product) {
    const card = document.createElement('div');
    card.classList.add('cardN');
...
function attachCartEventsNew() {
    document.querySelectorAll('.New_product .add-to-cart').forEach(btn => {
        btn.removeEventListener('click', handleNewAdd);
        btn.addEventListener('click', handleNewAdd);
    });
    document.querySelectorAll('.New_product .one-click-buy').forEach(btn => {
        btn.removeEventListener('click', () => alert('Заглушка 1 клик'));
        btn.addEventListener('click', () => alert('Заглушка 1 клик'));
    });
}
...
fetch(NEW_API)
    ...
    .then(data => {
        ...
        products.forEach(product => {
            newContainer.appendChild(createNewCard(product));
        });
        attachCartEventsNew();
        if (typeof initNewSlider === 'function') initNewSlider();
    })
